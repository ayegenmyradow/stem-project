...
app.post("/login", (req, res) => {
  ...
  if (!body.phone.match(/^\+9936[0-6][0-9]{6}$/)){
    res.render("login.html", {title: "Stem Group - login", errorMessage: "phone is not correct"});
    return
  }
  const query = "SELECT u.user_id, u.phone, u.password, u.name, u.surname FROM tbl_users u WHERE u.phone=?;"
  db.query(query, [body.phone], function(err, result){
    if (err)  {
      res.render("login.html", {title: "Stem Group - login", errorMessage: err.message});
    } else {
      if (result.length === 0){
        res.render("login.html", {title: "Stem Group - login", errorMessage: "User not found"});
        return;
      }
      const user = result[0];
      if (user.password !== body.password){
        res.render("login.html", {title: "Stem Group - login", errorMessage: "Password is not correct"});
        return;
      }
      res.cookie("token", generateToken(user.user_id, Config.maxAge));
      res.cookie("name", user.name, {maxAge: 1000 * 60 * Config.maxAge});
      res.cookie("surname", user.surname, {maxAge: 1000 * 60 * Config.maxAge});
      res.cookie("phone", user.phone, {maxAge: 1000 * 60 * Config.maxAge});
      res.cookie("user_id", user.user_id, {maxAge: 1000 * 60 * Config.maxAge});
      res.redirect("/")
    }
  })
});
...
app.post("/register", (req, res) => {
  ...
  const query = "INSERT INTO tbl_users(name, surname, email, phone, password) VALUES(?, ?, ?, ?, ?)"
  db.query(query, [body.name, body.surname, body.email, body.phone, body.password], function(err, result){
    if (err)  {
      ...
      res.cookie("surname", body.surname, {maxAge: 1000 * 60 * Config.maxAge});
      res.cookie("phone", body.phone, {maxAge: 1000 * 60 * Config.maxAge});
      res.cookie("user_id", body.user_id, {maxAge: 1000 * 60 * Config.maxAge});
      res.redirect("/")
    }
  })
});
...
app.get("/codes", async (req, res) => {
  const user = await getUsername(db, req.credentials.user_id)
  res.render("codes.html", {users: [], title: "Stem Group", username: user, active: "codes"});
});
app.get("/sql", async (req, res) => {
  ...
  if (req.query.command){
    ...
    db.query(decodeURIComponent(req.query.command), function(err, result){
      res.render("sql.html", {
        err, result: Array.isArray(result) ? result : [result], 
        title: "Stem Group", 
        username: user,  
    ...
    res.render("sql.html", {
      ...
      active: "sql",
      result: [], 
      title: "Stem Group", username: user, 
      command: req.query.command||""});
  }
});
